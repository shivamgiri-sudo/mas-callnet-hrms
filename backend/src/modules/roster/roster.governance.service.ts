...
export const rosterGovernanceService = {
  ...
  async createShiftTemplate(input: CreateShiftTemplateInput, userId: string, req?: Request): Promise<ShiftTemplate> {
    if (!input.shift_code || !input.shift_name || !input.start_time || !input.end_time || !input.effective_from) {
      throw Object.assign(new Error("shift_code, shift_name, start_time, end_time, effective_from are required"), { statusCode: 400 });
    }
    const id = randomUUID();
    await db.execute(
    ...
    await logSensitiveAction({ actor_user_id: userId, action_type: "shift_template_create", module_key: "roster_gov", entity_type: "wfm_shift_template", entity_id: id, req });
  ...
  async createCycle(input: CreateCycleInput, userId: string, req?: Request): Promise<RosterCycle> {
    ...
    await logSensitiveAction({ actor_user_id: userId, action_type: "cycle_create", module_key: "roster_gov", entity_type: "weekly_roster_cycle", entity_id: id, req });
  ...
  async advanceCycleStatus(id: string, newStatus: string, userId: string, req?: Request): Promise<RosterCycle> {
    ...
    if (!allowed.includes(newStatus)) {
      throw Object.assign(
        new Error(`Invalid transition: ${cycle.status} → ${newStatus}. Allowed: ${allowed.join(", ") || "none"}`),
        { statusCode: 400 }
      );
    }
    ...
    await logSensitiveAction({
      ...
      action_type: "cycle_status_advance",
      module_key: "roster_gov",
      entity_type: "weekly_roster_cycle",
      entity_id: id,
      change_summary: { from: cycle.status, to: newStatus },
      req,
    });
  ...
  async bulkUpsertAssignments(cycleId: string, assignments: BulkAssignRow[], userId: string, req?: Request): Promise<{ upserted: number; errors: string[] }> {
    await this.getCycle(cycleId); // validate cycle exists
    let upserted = 0;
    const errors: string[] = [];
    ...
    await logSensitiveAction({
      actor_user_id: userId,
      action_type: "assignments_bulk_upsert",
      module_key: "roster_gov",
      entity_type: "weekly_roster_cycle",
      entity_id: cycleId,
      change_summary: { upserted, errors: errors.length },
      req,
    });
  ...
  async acknowledgeRoster(cycleId: string, employeeId: string, userId: string, req?: Request): Promise<{ acknowledged: number }> {
    ...
    await logSensitiveAction({
      actor_user_id: userId,
      action_type: "roster_acknowledge",
      module_key: "roster_gov",
      entity_type: "weekly_roster_cycle",
      entity_id: cycleId,
      change_summary: { employee_id: employeeId, acknowledged: (result as { affectedRows: number }).affectedRows },
      req,
    });
    return { acknowledged: (result as { affectedRows: number }).affectedRows };
  },
  ...
  async logRosterChange(cycleId: string, input: CreateChangeLogInput, userId: string, req?: Request): Promise<ChangeLog> {
    await this.getCycle(cycleId); // validate
    ...
    await logSensitiveAction({
      actor_user_id: userId,
      action_type: "roster_change_log",
      module_key: "roster_gov",
      entity_type: "roster_change_log",
      entity_id: id,
      change_summary: { cycle_id: cycleId, employee_id: input.employee_id, change_type: input.change_type },
      req,
    });
  ...
  async createCoverageAction(input: CreateCoverageActionInput, userId: string, req?: Request): Promise<CoverageAction> {
    if (!input.cycle_id || !input.action_date) {
      throw Object.assign(new Error("cycle_id, action_date are required"), { statusCode: 400 });
    }
    await this.getCycle(input.cycle_id);
    const id = randomUUID();
    await db.execute(
    ...
    await logSensitiveAction({ actor_user_id: userId, action_type: "coverage_action_create", module_key: "roster_gov", entity_type: "roster_coverage_action", entity_id: id, req });
  ...
  async resolveCoverageAction(id: string, userId: string, req?: Request): Promise<CoverageAction> {
    const [existing] = await db.execute<CoverageAction[]>("SELECT * FROM roster_coverage_action WHERE id = ? LIMIT 1", [id]);
    if (!existing[0]) throw Object.assign(new Error("Coverage action not found"), { statusCode: 404 });
    const now = new Date().toISOString().slice(0, 19).replace("T", " ");
    await db.execute(
      "UPDATE roster_coverage_action SET status = 'resolved', resolved_at = ? WHERE id = ?",
      [now, id]
    );
    await logSensitiveAction({ actor_user_id: userId, action_type: "coverage_action_resolve", module_key: "roster_gov", entity_type: "roster_coverage_action", entity_id: id, req });
    const [rows] = await db.execute<CoverageAction[]>("SELECT * FROM roster_coverage_action WHERE id = ? LIMIT 1", [id]);
    return rows[0];
  },
