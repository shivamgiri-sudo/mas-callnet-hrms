...
router.get("/team-kpi", requireRole("admin","hr","manager"), h(async (req: AuthenticatedRequest, res: Response) => {
...
router.get("/coaching", h(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (await hasRole(userId, "admin","hr","manager")) {
...
router.post("/coaching", requireRole("admin","hr","manager"), h(async (req: AuthenticatedRequest, res: Response) => {
...
router.get("/alerts", requireRole("admin","hr","manager"), h(async (req: AuthenticatedRequest, res: Response) => {
...
router.post("/alerts/:id/acknowledge", requireRole("admin","hr","manager"), h(async (req: AuthenticatedRequest, res: Response) => {
  await managementService.acknowledgeAlert(req.params.id, req.authUser!.id, req);
  res.json({ ok: true });
}));

router.get("/dashboard", requireRole("admin","hr","manager"), h(async (req: AuthenticatedRequest, res: Response) => {
