...
| Phase | Module | Task | Current State | Planned Change | Files Affected | DB Impact | API Impact | Frontend Impact | Roles Impacted | Test Required | Risk | Status | Approval Required |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 3 | ATS | Manpower Requisition | Not present | `manpower_requisition` table + CRUD API + approval | `017_ats_wfm_completion.sql`, `ats-ext.service.ts`, `ats-ext.routes.ts` | Additive | `GET/POST /api/ats-ext/requisitions`, `POST /api/ats-ext/requisitions/:id/approve` | None | Admin, HR, Manager | 3 tests passing; recruiter 403; audit on create/approve | Medium | ✅ DONE | Package 3 |
...
| 3 | ATS | Duplicate Detection | Not present | `ats_duplicate_log` table + check/resolve API; additive columns on `ats_candidate` | `017_ats_wfm_completion.sql`, `ats-ext.service.ts` | Additive | `GET /api/ats-ext/duplicates`, `POST /api/ats-ext/duplicates/:id/resolve` | None | Admin, HR, Recruiter | Schema + service ready | Low | ✅ DONE | Package 3 |
| 3 | ATS | Sourcing Funnel Analytics | Not present | Aggregation queries on `ats_candidate` by sourcing_channel + stage | `ats-ext.service.ts`, `ats-ext.routes.ts` | Read-only | `GET /api/ats-ext/analytics/funnel`, `GET /api/ats-ext/analytics/stages` | Existing ATS dashboard data source | Admin, HR, Recruiter, Manager | 2 tests passing | Low | ✅ DONE | Package 3 |
...
| 3 | WFM | Roster Swap Requests | Not present | `wfm_roster_swap_request` table + create/review API | `017_ats_wfm_completion.sql`, `wfm-ext.service.ts`, `wfm-ext.routes.ts` | Additive | `GET/POST /api/wfm-ext/roster/swaps`, `POST /api/wfm-ext/roster/swaps/:id/review` | None | Any (create own), Admin/HR/WFM/Manager (review) | 3 tests; employee scope enforced | Low | ✅ DONE | Package 3 |
| 3 | WFM | Roster Conflict Log | Not present | `wfm_roster_conflict_log` table + log/list/resolve | `017_ats_wfm_completion.sql`, `wfm-ext.service.ts` | Additive | `GET /api/wfm-ext/roster/conflicts`, `POST /api/wfm-ext/roster/conflicts/:id/resolve` | None | Admin, HR, WFM, Manager | Schema + service ready | Low | ✅ DONE | Package 3 |
| 3 | WFM | Coverage / Shrinkage Snapshots | Not present | `wfm_coverage_snapshot` table + upsert/query API | `017_ats_wfm_completion.sql`, `wfm-ext.service.ts` | Additive | `GET /api/wfm-ext/coverage`, `POST /api/wfm-ext/coverage/snapshot` | None | Admin/WFM (write), Admin/HR/WFM/Manager (read) | 2 tests; hr 403 on write; shrinkage calculated | Low | ✅ DONE | Package 3 |
| 3 | WFM | Attrition Records + Summary | Not present | `attrition_record` table + record/summary API | `017_ats_wfm_completion.sql`, `wfm-ext.service.ts` | Additive | `POST /api/wfm-ext/attrition/record`, `GET /api/wfm-ext/attrition/summary` | None | Admin, HR | 3 tests; 400 on missing fields | Low | ✅ DONE | Package 3 |
